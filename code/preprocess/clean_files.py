...
# Fancyimpute now calls tensorflow so also setting seed
...
bg = bg.replace("Missing", np.NaN) # This errors in latest version so done separately
...
k = 5
print("Now running KNN imputation using ", k, " nearest neighbours...")
bg_imputed = KNN(k=k).fit_transform(bg_)
